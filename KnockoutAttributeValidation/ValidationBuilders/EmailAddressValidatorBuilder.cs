...
{
    public class EmailAddressValidatorBuilder : IValidatorBuilder
    {
        public Type ValidationAttributeType { get { return typeof(EmailAddressAttribute); } }

        public IEnumerable<IValidator> Build(ValidationAttribute attribute)
        {
            var minLengthAttribute = (EmailAddressAttribute)attribute;

            var validator = new RegularExpressionValidator()
            {
                ErrorMessage = minLengthAttribute.ErrorMessage,
                //this pattern was grabbed from microsoft source: https://github.com/ASP-NET-MVC/aspnetwebstack/blob/master/src/Microsoft.Web.Mvc/EmailAddressAttribute.cs
